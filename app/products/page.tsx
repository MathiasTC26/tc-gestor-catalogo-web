...
const mockProducts: ProductRecord[] = [
  {
    id: "P-1001",
    name: "Máquina recta industrial",
    code: "TC-RECTA-01",
    factoryCode: "R-8700",
    brand: "Todo Costura",
    category: "Máquina recta",
    price: "2.850.000",
    description:
      "Máquina recta industrial para costura continua.\nIdeal para talleres de producción.\nAlta velocidad y estructura reforzada.",
  },
  {
    id: "P-1002",
    name: "Overlock 5 hilos",
    code: "TC-OVER-05",
    factoryCode: "O-5500",
    brand: "Todo Costura",
    category: "Overlock",
    price: "3.420.000",
    description:
      "Overlock de 5 hilos para terminación profesional.\nCostura firme, limpia y de alto rendimiento.",
  },
  {
    id: "P-1003",
    ...
    price: "3.100.000",
    description:
      "Collareta industrial para terminaciones de prendas.\nApta para tejidos livianos y medianos.",
  },
];
...
export default function ProductsPage() {
  ...
  const groupedProducts = useMemo(() => {
    ...
    for (const product of addedProducts) {
      const existing = groups.get(product.sheet) ?? [];
      existing.push(product);
      groups.set(product.sheet, existing);
    }
          ...
          <section className="mt-4 rounded-[25px] border border-[#bdb5b9] bg-[linear-gradient(180deg,#e9e4e6,#ded9db)] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,.52)] sm:p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              ...
              <div className="grid grid-cols-2 gap-2 sm:min-w-[260px]">
                ...
                <InfoChip label="Artículos" value={`${addedProducts.length}/${revista.maxArticles}`} />
              </div>
            </div>
          </section>
                      ...
                      <section key={group.sheetNumber} className="grid gap-2">
                        ...
                        <div className="grid gap-2">
                          {group.products.map((product) => {
                            const label = getLabel(product.labelId);
  ...
  return (
    ...
    >
      <div className="mb-3 flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h4 className="truncate text-[13px] font-black text-[#241f22]">{product.name}</h4>
  ...
  return (
    <article className="overflow-hidden rounded-[16px] border border-[#bdb5b9] bg-[#e9e4e6] shadow-[inset_0_1px_0_rgba(255,255,255,.45)]">
      ...
      <div className="flex gap-3 p-3">
        ...
        <div className="min-w-0 flex-1">
          <h4 className="truncate text-[12.5px] font-black text-[#241f22]">{product.name}</h4>
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-white/10 bg-[linear-gradient(to_top,rgba(16,16,17,.98)_70%,rgba(16,16,17,.78)_88%,transparent)] px-3 pb-[calc(12px+env(safe-area-inset-bottom))] pt-5 backdrop-blur-md lg:hidden">
      <div className="mx-auto grid max-w-[440px] gap-2">
        ...
        <button className="tc-primary-button flex w-full items-center justify-center gap-2">
          Ver previa y guardar revista
          <ArrowRight className="h-4 w-4" />
        </button>
      </div>
